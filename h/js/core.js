...
const anita_config = {
    ...
    main_menu: {
        'Главная':'https://forestkzn.com' ,
        'Генплан': 'https://forestkzn.com/map.php',
        'Проекты домов': '/projects.html',
        'Готовые дома': '/?houses',
        'Ход строительства': '/timeline.html'
    },
    ...
    socials: {
        'facebook' : {
            'url': 'https://vk.com/vm_stroy',
            'label': 'VK'
        },
      
        'instagram' : {
            'url': 'https://www.instagram.com/forest_kazan_rt',
            'label': 'Instagram'
        },
        'youtube' : {
            'url': 'https://youtube.com/@vmstroy?si=bX0a1x6jS0RiWEW9',
            'label': 'Youtube'
        },
          'telegram' : {
            'url': 'https://vm-stroy.pro',
            'label': 'Сайт'
        },
    },
    ...
    l10n: {
        ...
        copyright: 'КП FOREST 2024',

        // The message that appears when visitors try to open context menu
  rcp_message: 'Контекстное меню отключено на этом сайте',
                rcp_button: 'ОК',
                b2t_label: 'Вернуться назад'
    }
}
...
class Anita {
    ...
    contentOrigin() {
        if ( this.$el.main.children('.anita-container').length ) {
            this.$el.main.children('.anita-container').css('transform-origin', '100% ' + (window.scrollY + 0.5 * window.innerHeight) + 'px');
        }
    }
    generateID() {
            		...
            		if ($this.attr('data-type') !== 'image') {
            			...
            			if ( 'video' == $this.attr('data-type') ) {
            				...
            				if ( $this_video.children('iframe').length ) {
                                $this_video.children('iframe').attr('src', $this_video.attr('data-player-src')+'?controls=1&amp;loop=0&amp;autoplay=1&amp;muted=0');
            				}
            			}
            		}
    	...
    	if (jQuery('.anita-page-background[data-src]:not(.is-loaded)').length) {
    		jQuery('.anita-page-background[data-src]:not(.is-loaded)').each(function() {
                ...
                if ($this.hasClass('is-video')) {
                    $this.append('<video src="'+ $this.attr('data-src') +'" webkit-playsinline="true" playsinline="true" autoplay loop/>').addClass('is-loaded');
                } else {
                    $this.css('background-image', 'url('+ $this.attr('data-src') +')').addClass('is-loaded');
                }
